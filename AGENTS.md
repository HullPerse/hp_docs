...
- **Chat answers are terse.** Result first, no polite filler, short sentences. Task reports keep the mandatory structure; brevity never hides a decision or a verification result.
...
- **Agents run in parallel.** Other sessions may commit while you work. Sync four times: on start (check the working tree for foreign changes), before questions (re-check the DECISIONS.md tail), before writes (re-read target files loaded earlier), on finish (append decisions and list changed files first). Full protocol: `.docs/AGENT_PROMPT.md`, "Parallel sessions".
